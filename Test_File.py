...
def main_app():
    ...
    if selected_menu == "🏢 센터 소개":
        ...
        st.markdown("---")
        st.markdown("<h3 style='font-size: 18px; font-weight: bold; color: inherit;'>📱 스마트폰에 1초 만에 센터 앱 설치하기</h3>", unsafe_allow_html=True)
        st.markdown("<div style='font-size: 14px; background-color: #e8f4f8; padding: 15px; border-radius: 8px; color: #002147; margin-bottom: 15px; border-left: 5px solid #002147;'>💡 이 홈페이지를 스마트폰 바탕화면에 추가해두면 어플처럼 터치 한 번으로 접속할 수 있습니다.</div>", unsafe_allow_html=True)
        
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("<div style='font-size: 15px; font-weight: bold; margin-bottom: 8px; color: inherit;'>🍎 아이폰 (Safari)</div>", unsafe_allow_html=True)
            st.markdown("<div style='font-size: 13px; line-height: 1.6; color: inherit;'>1. 화면 하단의 <b>공유</b>(네모에 화살표) 버튼 터치<br>2. 메뉴를 위로 올려 <b>'홈 화면에 추가'</b> 선택<br>3. 우측 상단 <b>'추가'</b> 버튼 터치</div>", unsafe_allow_html=True)
        with col2:
            st.markdown("<div style='font-size: 15px; font-weight: bold; margin-bottom: 8px; color: inherit;'>🤖 안드로이드 (Chrome)</div>", unsafe_allow_html=True)
            st.markdown("<div style='font-size: 13px; line-height: 1.6; color: inherit;'>1. 우측 상단의 <b>메뉴</b>(점 3개) 버튼 터치<br>2. <b>'홈 화면에 추가'</b> 선택<br>3. 팝업창에서 <b>'추가'</b> 버튼 터치</div>", unsafe_allow_html=True)
